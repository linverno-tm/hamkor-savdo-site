...
echo '<li>' . $okIco($aiKey !== '') . '<div><b>' . ($aiKey !== '' ? 'Tahlil: ' . (hs_mb_provider() === 'gemini' ? 'Gemini' : 'Claude') : 'AI kaliti yo\'q') . '</b><small>'
    . ($aiKey !== '' ? 'Telegram bo\'limidagi o\'sha kalit ishlatiladi — alohida kalit kerak emas. <a href="/admin/telegram.php?bolim=ai#mb_key">Kalitni ochish</a>.' . ($xato ? ' Oxirgi xato: ' . h($xato) : '')
        : 'Kalitsiz e\'lonlar yig\'iladi, lekin tahlil qilinmaydi. Kalit: <a href="/admin/telegram.php?bolim=ai#mb_key">Telegram bo\'limida</a>.') . '</small></div></li>';
echo '</ul>';
