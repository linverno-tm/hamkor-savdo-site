...
export function BranchMap({ branches, apiKey }: { branches: MapBranch[]; apiKey: string }) {
  ...
  useEffect(() => {
    ...
    const select = (id: string) => {
      ...
      marks.forEach((pm, key) => pm.properties.set("active", key === id));
    };
    ...
    const onFocusClick = (e: MouseEvent) => {
      ...
      e.preventDefault();
      select(b.id);
      void map.setCenter([b.lat, b.lng], 16, { duration: 400 });
      pm.balloon.open();
    ...
    const init = () =>
      loadYmaps(apiKey)
        .then((ymaps) => {
          if (cancelled) return;
          map = new ymaps.Map(el, { center: [40.75, 72.2], zoom: 9, controls: [] }, { suppressMapOpenBlock: true });
          ...
          for (const b of branches) {
            const pm = new ymaps.Placemark(
              [b.lat, b.lng],
              {
                ...
                index: b.index,
                active: false,
                balloonContentHeader: esc(b.title),
                balloonContentBody: `${esc(b.address)}<br>Ish vaqti: ${esc(b.hours)}`,
                balloonContentFooter: `<a href="${b.href}">Filial sahifasi →</a>`,
              },
              {
                iconLayout: pin,
                iconShape: { type: "Circle", coordinates: [0, -22], radius: 22 },
                hideIconOnBalloonOpen: false,
                balloonOffset: [0, -44],
                openHintOnHover: false,
              },
            );
            pm.events.add("click", () => select(b.id));
